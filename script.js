    ...
    const processDraw = async (q = "", isDaily = false, isReread = false, savedState = null) => {
        ...
        extraHtml += `
            <div class="mt-4 d-flex flex-wrap justify-content-center gap-2" data-html2canvas-ignore="true">
                ...
                <button id="copy-btn" class="btn btn-sm btn-outline-secondary rounded-pill px-3">🤖 複製去問 AI</button>
            </div>`;
        ...
        setTimeout(() => {
            ...
            if (copyBtn) {
                ...
                // 取 modalBody 純文字，只保留牌面解釋和具體建議，去掉按鈕和音樂資訊
                const rawText = modalBody ? modalBody.innerText : '';
                // 截到幸運物/幸運色/推薦曲目之前，或保留到建議結束
                const cutPatterns = ['🍀', '✨ 幸運色', '🎵 宇宙', '📖', '🔄', '🤖'];
                let aiReading = rawText;
                for (const pat of cutPatterns) {
                    const idx = aiReading.indexOf(pat);
                    if (idx !== -1) aiReading = aiReading.substring(0, idx);
                }
